...
max_alpha = 2e-6 #Maximum thermal diffusivity[m^2/s]
...
k_val = 1000 #Thermal conductivity [W/mK]
k_arr = np.array([k_val for _ in range(Nalpha)]) #Thermal conductivity 
C_arr = k_arr/alpha #Volumetric heat capacity

# System length [m]
min_L = 0.02 #Minimum length[m]
max_L = 0.2 #Maximum length[m]
...
PercentageError2 = np.zeros((Nalpha,NL))
# Run
for alpha_idx in tqdm(range(Nalpha)): #ThermalDiffusivity(y axis)
    ...
    for Lidx in range(NL): #L(x axis)
        ...
        # print(f"PercentageError2: {Error}%")
        PercentageError2[alpha_idx,Lidx] = Error #Save in dataframe

k2Error = arr2df(PercentageError2)
